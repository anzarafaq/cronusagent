...
def removeAgentRoot():
    ...
    if (not os.name == 'nt'):
        ...
        try:
            #shutil.rmtree(pylons.config['agent_root'])
            cmd = 'rm -rf %s' % pylons.config['agent_root']
            os.system(cmd)
            return True
...
def activateManifest(testController, manifest = 'bar', service = 'foo'):
    ...
    response = testController.app.post(url(controller = 'action', action = 'activatemanifest', service = service),
                                       headers = {'Content-Type' : 'application/json'},
                                       params = body)
    ...
    LOG.debug ('link = ' + link)
    if os.name == 'nt':
        manifestPath = os.path.join(ServiceController.manifestPath(service), manifest)
