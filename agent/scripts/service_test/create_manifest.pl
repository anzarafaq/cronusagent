...
my $request = '{"package":["bittorrent://qa-slab003.qa.ebay.com/csa-2.0.1.win.cronus","http://qa-slab003.qa.ebay.com/ebox-1.0.0.win.cronus"]}';
...
if($status =~ m/\/status\/(.*?)\"/) {
	...
	for (my $count = 1; $count < 180; $count++) {
